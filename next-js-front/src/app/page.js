...
import { useClient, useNavigation, useSearchParams } from "next/client";
...
export default function Page() {
  useClient(); // Marca o componente pai com "useClient"
  ...
  return (
    <div>
      <button className="add-button" onClick={() => navigation.push("/add")}>
        Add
      </button>
      {displaySuccessMessage.show && (
      ...
      {menuItems ? (
        menuItems.map((item) => (
          <MenuItem
            ...
            name={item.name}
            price={item.price}
            onEdit={() => navigation.push(`/update/${item.id}`)}
